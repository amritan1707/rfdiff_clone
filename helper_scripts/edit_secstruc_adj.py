...
def main():
    ...
    name=os.path.split(args.input_pdb)[1][:-4]
    secstruc_dict=extract_secstruc(args.input_pdb)
    xyz=secstruc_dict['xyz']
    ss, idx = ss_to_tensor(secstruc_dict)
    block_adj = construct_block_adj_matrix(torch.FloatTensor(ss), torch.tensor(xyz)).float()

    # Update ss and adj matrix
    if len(args.update_ss) > 0:
      ss = update_ss(ss, secstruc_dict["pdb_idx"], args.update_ss)
      block_adj = update_adj(block_adj, ss, secstruc_dict["pdb_idx"], args.update_ss)
...
def get_args():
    ...
    parser.add_argument("--input_pdb", help="path to input pdb")
    parser.add_argument("--out_dir", help='need to specify an output path')
    parser.add_argument("--update_ss", nargs='*', default=[], help='residues which to update ss and adj matrix. '
                        'For example, to modify residues A10 through A20 to be all helical and adjacent '
                        'to ss of residue A50, input "A10-20:H:A50". Note you can input multiple of these to '
...
def update_ss(ss, pdb_idx, update_ss):
  """
  Function to update ss based on user input. 
  Input:
    ss: tensor of shape [L,4] where L is the length of the protein and 4 is the one-hot encoding of ss
    pdb_idx: list of tuples of shape [L] where L is the length of the protein and each tuple is the chain and residue number
    update_ss: list of strings of shape [N] where N is the number of regions to update. Each string is of the form "A10-20:H:A50" where A10-20 is the region to update, H is the ss to update to, and A50 is the ss to be adjacent to. 
  Output:
    ss: tensor of shape [L,4] where L is the length of the protein and 4 is the one-hot encoding of ss
  """
  ss_conv = {'H':0,'E':1,'L':2}

  for update in update_ss:
    # Parse input
    split_update = update.split(':')
    if len(split_update) < 2 or len(split_update) > 3:
      raise ValueError('update_ss must be of the form "A10-20:H:A50" or "A10-20:H"')
    region = split_update[0]
    ss_to_update = split_update[1]

    # Find indices of residues to update
    chain = region[0]
    if '-' not in region:
      start = int(region[1:])
      end = start
    ...
    ss_to_update = int(ss_conv[ss_to_update])

    # Find indices of residues to update
    indices = []
    for i in range(len(pdb_idx)):
      if pdb_idx[i][0] == chain and pdb_idx[i][1] >= start and pdb_idx[i][1] <= end:
        indices.append(i)

    # Update ss
    ss[indices] = ss_to_update

  return ss


def update_adj(adj, ss, pdb_idx, update_ss):
    """
    Function to update adj based on user input. 
    Input:
      adj: tensor of shape [L,L] where L is the length of the protein
      ss: tensor of shape [L,4] where L is the length of the protein and 4 is the one-hot encoding of ss
      pdb_idx: list of tuples of shape [L] where L is the length of the protein and each tuple is the chain and residue number
      update_ss: list of strings of shape [N] where N is the number of regions to update. Each string is of the form "A10-20:H:A50" where A10-20 is the region to update, H is the ss to update to, and A50 is the ss to be adjacent to. 
    Output:
    ...
    for i in range(L):
        ...
        if not ss[i] == ss[i-1]:
            ...
            segments.append( (ss[i-1], begin, end) )

            begin = i

    # Ending edge case: last segment is length one
    if not end == ss.shape[0]:
        segments.append( (ss[-1], begin, ss.shape[0]) )
    ...
    for update in update_ss:
      ...
      if len(split_update) < 2 or len(split_update) > 3:
        raise ValueError('update_ss must be of the form "A10-20:H:A50" or "A10-20:H"')
      elif len(split_update) == 2:
        adj_updates = []
      else:
        adj_updates = [(i[0], int(i[1:])) for i in split_update[2].split(',')]
      region = split_update[0]
      ss_to_update = split_update[1]
      ...
      # Find indices of adj residues
      adj_indices = []
      for i in range(len(pdb_idx)):
        if pdb_idx[i] in adj_updates:
          adj_indices.append(i)
      
      for seg in segments:
        if seg[2] < min(indices):
          continue
        elif seg[1] > max(indices):
          continue
        else:
          update_seg = seg
          break
      ...
      for adj_i in adj_indices:
        for seg in segments:
          if adj_i >= seg[1] and adj_i < seg[2]:
            adj_segs.append(seg)
            break

      adj[update_seg[1]:update_seg[2], :] = 0
      adj[:, update_seg[1]:update_seg[2]] = 0
      if ss_to_update != 'L':
        for adj_seg in adj_segs:
          if adj_seg[0] == 2: # Don't connect to loops
             continue
          adj[update_seg[1]:update_seg[2], adj_seg[1]:adj_seg[2]] = 1
          adj[adj_seg[1]:adj_seg[2], update_seg[1]:update_seg[2]] = 1
