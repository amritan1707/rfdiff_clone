...
from rfdiff_clone.rfdiffusion.RoseTTAFoldModel import RoseTTAFoldModule
from rfdiff_clone.rfdiffusion.kinematics import get_init_xyz, xyz_to_t2d
from rfdiff_clone.rfdiffusion.diffusion import Diffuser
from rfdiff_clone.rfdiffusion.chemical import seq2chars
from rfdiff_clone.rfdiffusion.util_module import ComputeAllAtomCoords
from rfdiff_clone.rfdiffusion.contigs import ContigMap
from rfdiff_clone.rfdiffusion.inference import utils as iu, symmetry
from rfdiff_clone.rfdiffusion.potentials.manager import PotentialManager
import logging
import torch.nn.functional as nn
from rfdiff_clone.rfdiffusion import util
from hydra.core.hydra_config import HydraConfig
import os

from rfdiff_clone.rfdiffusion.model_input_logger import pickle_function_call
import sys
...
class Sampler:
    ...
    def construct_contig(self, target_feats, contig_2=False):
        """
        ...
        if contig_2:
            #replace contig with contig_2
            duplicate_dict = self.contig_conf.copy()
            duplicate_dict['contigs'] = duplicate_dict['contigs_2']
            duplicate_dict.pop('contigs_2')
            
            return ContigMap(target_feats, **duplicate_dict, sym_order=sym_order)
        
        else:
            #remove contigs_2 from duplicate_dict
            duplicate_dict = self.contig_conf.copy()
            duplicate_dict.pop('contigs_2')
    ...
    def sample_init(self, return_forward_trajectory=False, translate=False):
        ...
        if self.contig_conf.contigs_2 is not None:
            self.contig_map_2 = self.construct_contig(self.target_feats, contig_2=True)
            self.mappings_2 = self.contig_map_2.get_mappings()
            self.mask_seq_2 = torch.from_numpy(self.contig_map_2.inpaint_seq)[None,:]
            self.mask_str_2 = torch.from_numpy(self.contig_map_2.inpaint_str)[None,:]
            self.binderlen_2 =  len(self.contig_map_2.inpaint)  

        #print(self.mask_str.squeeze(), self.mask_str_2.squeeze())
        ####################
        ### Get Hotspots ###
        ####################
        ...
        if self.contig_conf.contigs_2 is not None:
            self.diffusion_mask_2 = self.mask_str_2
        ...
        if self.diffuser_conf.partial_T:
            ...
            if translate:
                #option 1:
                #get coordinates for residues that being diffused (binder) from all coordinates (xyz_27) and translate them in random direction
                xyz_translate = xyz_mapped[[not x for x in self.diffusion_mask.squeeze()]]
                
                #out = "/work/users/a/m/amritan/partial_diff/testRFdiff/tests/binder_cd19/temp1.pdb"
                #util.writepdb(
                #    out,
                #    xyz_mapped[:, :4, :],
                #    seq_orig,
                ...
                #out = "/work/users/a/m/amritan/partial_diff/testRFdiff/tests/binder_cd19/temp2.pdb"
                #util.writepdb(
                #    out,
                #    xyz_mapped[:, :4, :],
                #    seq_orig,
                ...
                #option 2: TODO
                #get coordinates for residues that being diffused (binder) from all coordinates (xyz_27)
                #get 3 target residues at interface
                #compute a plane
                #choose random direction on plane and move binder coords in that direction
        ...
        if self.diffuser_conf.partial_T:
            ...
            self.t_step_input = int(self.diffuser_conf.partial_T)
            if self.contig_conf.contigs_2 is not None:
                self.t_step_input_2 = int(self.diffuser_conf.partial_T2)
        else:
            self.t_step_input = int(self.diffuser_conf.T)
        t_list = np.arange(1, self.t_step_input+1)
        if self.contig_conf.contigs_2 is not None:
            t_list_2 = np.arange(1, self.t_step_input_2+1)
        ...
        if self.diffusion_mask_2 is not None:
            ...
            t_list=t_list,
            diffusion_masks=(self.diffusion_mask.squeeze(), self.diffusion_mask_2.squeeze()),
            t_lists=(t_list, t_list_2))
        ...
        self._log.info(f'Sequence init HERE: {seq2chars(torch.argmax(seq_t, dim=-1))}')
...
class ScaffoldedSampler(SelfConditioning):
    ...
    def sample_init(self):
        ...
        if self.diffuser_conf.partial_T:
            assert self.diffuser_conf.partial_T <= self.diffuser_conf.T
            self.t_step_input = int(self.diffuser_conf.partial_T)
            if self.diffuser_conf.partial_T2:
                assert self.diffuser_conf.partial_T2 <= self.diffuser_conf.T
                self.t_step_input_2 = int(self.diffuser_conf.partial_T2)
        else:
            self.t_step_input = int(self.diffuser_conf.T)
        t_list = np.arange(1, self.t_step_input+1)
