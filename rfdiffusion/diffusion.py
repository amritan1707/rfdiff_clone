...
class Diffuser:
    ...
    def diffuse_pose(
        ...
        if diffusion_mask is None or diffusion_masks is not None:
        ...
        inv_all_diffusion_mask=None
        diffused_fa = None
        #list_lens_tlists = [len(i) for i in t_lists]
        #t_lists_longest = t_lists[np.argmax(np.array(list_lens_tlists))]
        if diffusion_masks is not None:
            #print(t_lists, t_lists_longest)
            #diffused_fa = torch.zeros(t_lists_longest, L, 27, 3)
            for diffusion_mask in diffusion_masks:
                inv_diffusion_mask = [not elem for elem in diffusion_mask]
                if inv_all_diffusion_mask is not None:
                    inv_all_diffusion_mask = np.array(inv_diffusion_mask) | inv_all_diffusion_mask
                else:
                    inv_all_diffusion_mask = np.array(inv_diffusion_mask)
                ...
                if diffused_fa is None:
                    diffused_fa = torch.zeros(t_steps, L, 27, 3).float()
                #print(t_steps)
                #print(diffusion_mask)
                #print(diffused_fa)
                diffused_fa[:, inv_diffusion_mask, :3, :] = diffused_BB[:, inv_diffusion_mask, :, :].float()
                #print(diffused_fa)

            # Add in sidechains from motif
            all_diffusion_mask = [not elem for elem in inv_all_diffusion_mask]
            if include_motif_sidechains:
                diffused_fa[:, all_diffusion_mask, :14, :] = xyz_true[None, all_diffusion_mask, :14].float()
        ...
        if t_lists is not None and t_list is None:
            t_idx_lists = [[t - 1 for t in t_list] for t_list in t_lists]
            list_lens = [len(i) for i in t_idx_lists]
            t_idx_list = t_idx_lists[np.argmax(np.array(list_lens))]
            fa_stack = diffused_fa[t_idx_list]
        
        elif t_list is None and t_lists is None:
